...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(os.path.dirname(__file__), 'restore.log'))
    ]
)
...
def run_pg_restore(db_name, db_user, db_password, db_host, db_port, backup_file):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            logger.warning(f"pg_restore completed with warnings or errors: {result.stderr}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='PostgreSQL Database Restore Script')
    parser.add_argument('--backup-dir', help='Directory containing backups', default=os.path.join(BASE_DIR, 'backups'))
    parser.add_argument('--backup-file', help='Specific backup file to restore (optional)')
    parser.add_argument('--env', choices=['dev', 'prod'], help='Environment to restore to', default='dev')
    ...
    db_name = env_vars.get(f'DB_NAME_{args.env.upper()}') if args.env != 'prod' else env_vars.get('DB_NAME')
    db_user = env_vars.get(f'DB_USER_{args.env.upper()}') if args.env != 'prod' else env_vars.get('DB_USER')
    db_password = env_vars.get(f'DB_PASSWORD_{args.env.upper()}') if args.env != 'prod' else env_vars.get('DB_PASSWORD')
    db_host = env_vars.get(f'DB_HOST_{args.env.upper()}') if args.env != 'prod' else env_vars.get('DB_HOST')
    db_port = env_vars.get(f'DB_PORT_{args.env.upper()}') if args.env != 'prod' else env_vars.get('DB_PORT')
    ...
    if not backup_file:
        ...
        choice = input("Enter number of backup to restore (or press Enter for most recent): ")
        if choice.strip():
            try:
    ...
    confirm = input(f"Are you sure you want to restore database {db_name} from {os.path.basename(backup_file)}? [y/N]: ")
    if confirm.lower() != 'y':
        logger.info("Restore cancelled.")
        sys.exit(0)
    
    # Run pg_restore
    success = run_pg_restore(db_name, db_user, db_password, db_host, db_port, backup_file)
