...
def run_pg_dump(db_name, db_user, db_password, db_host, db_port, backup_file):
    ...
    try:
        ...
        try:
            pg_dump_version = subprocess.run(['pg_dump', '--version'], env=env, capture_output=True, text=True)
...
def main():
    ...
    parser.add_argument('--backup-dir', help='Directory to store backups', default=os.path.join(BASE_DIR, 'backups'))
    parser.add_argument('--keep-days', type=int, help='Number of days to keep backups', default=7)
    parser.add_argument('--env', choices=['dev', 'prod'], help='Environment to backup', default='dev')
    ...
    print(f"Database info - Name: {db_name}, User: {db_user}, Host: {db_host}, Port: {db_port}")
    
    # Generate backup filename with timestamp
    timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    backup_file = os.path.join(args.backup_dir, f"{db_name}_{timestamp}.backup")
    print(f"Backup file will be: {backup_file}")
    
    # Run pg_dump
    success = run_pg_dump(db_name, db_user, db_password, db_host, db_port, backup_file)
